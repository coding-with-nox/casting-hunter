...
import { getStoredInterval, setStoredInterval } from '../hooks/useAutoRefresh';

const INPUT = 'w-full bg-[#0a0a0a] border border-[#222] rounded-lg px-3 py-2 text-sm text-[#f5f5f5] placeholder-[#444] focus:outline-none focus:border-[#d4af37]/40';
...
export function Settings() {
  ...
  const [editForm, setEditForm] = useState({ url: '', region: 'Italy' });
  ...
  const startEdit = (s: SourceStatus) => {
    setEditingSource(s.name);
    setEditForm({ url: s.url ?? '', region: s.region });
  };

  const handleSaveEdit = async (name: string) => {
    setEditSaving(true);
    try {
      const updated = await castingApi.updateSource(name, {
        url: editForm.url || undefined,
        region: editForm.region,
      });
      setSources(prev => prev.map(s => s.name === name ? updated : s));
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="p-6 grid grid-cols-2 gap-5 min-h-full items-start" style={{ gridTemplateColumns: '1fr 1fr' }}>
        ...
        <div className="flex flex-col gap-5">
          ...
          <section className="bg-[#141414] border border-[#1e1e1e] rounded-xl p-5">
            <h3 className="text-xs font-semibold text-[#555] uppercase tracking-wider mb-4">Il tuo profilo</h3>
            <div className="flex flex-col gap-3">
              ...
              <div>
                ...
                <input type="number" value={form.scenicAge} onChange={e => setForm(f => ({ ...f, scenicAge: e.target.value }))}
                  placeholder="Es. 30" min={18} max={80} className={INPUT} />
              </div>
              <div>
                <label className="block text-xs text-[#666] mb-1">Telegram Chat ID</label>
                <input type="text" value={form.telegramChatId} onChange={e => setForm(f => ({ ...f, telegramChatId: e.target.value }))}
                  placeholder="Es. 123456789" className={INPUT} />
          ...
          <section className="bg-[#141414] border border-[#1e1e1e] rounded-xl p-5">
            ...
            <p className="text-xs text-[#444] mb-4">Ogni quanti secondi cercare nuovi casting in background.</p>
            <div className="flex items-center gap-3">
              <input type="number" min={10} max={3600} value={refreshInterval}
                onChange={e => setRefreshIntervalState(Number(e.target.value))}
                className="w-28 bg-[#0a0a0a] border border-[#222] rounded-lg px-3 py-2 text-sm text-[#f5f5f5] focus:outline-none focus:border-[#d4af37]/40" />
              ...
              <button onClick={handleSaveInterval}
                ...
                {intervalSaved ? '✓ Applicato' : 'Applica'}
              </button>
            </div>
            <p className="text-xs text-[#3a3a3a] mt-2">
              Il timer è visibile in alto a destra — clicca sull'icona per aggiornare subito.
            </p>
          </section>
        ...
        <div className="flex flex-col gap-5">
          ...
          <section className="bg-[#141414] border border-[#1e1e1e] rounded-xl p-5">
            ...
            <p className="text-xs text-[#444] mb-4">Hanno uno scraper dedicato e funzionano al meglio.</p>
            <div className="flex flex-col gap-2">
              {builtInSources.length === 0 && (
                <p className="text-xs text-[#444]">Nessun sito integrato trovato. Aggiorna per la prima volta per popolare la lista.</p>
              )}
              {builtInSources.map(s => (
                <div key={s.name} className="bg-[#0f0f0f] border border-[#1e1e1e] rounded-lg overflow-hidden">
                  {/* Riga principale */}
                  <div className="flex items-center gap-3 px-3 py-2.5">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-[#d5d5d5] font-medium">{s.name}</span>
                        <span className="text-xs text-[#3a7b47] bg-[#1a3a22] px-1.5 py-0.5 rounded">✓ Integrato</span>
                      </div>
                      {s.lastScrapedAt ? (
                        <p className="text-xs text-[#444] mt-0.5">
                          Aggiornato: {new Date(s.lastScrapedAt).toLocaleDateString('it-IT')}
                        </p>
                      ) : (
                        <p className="text-xs text-[#3a3a3a] mt-0.5">Mai aggiornato</p>
                      )}
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <button
                        onClick={() => editingSource === s.name ? setEditingSource(null) : startEdit(s)}
                        className="text-xs text-[#444] hover:text-[#d4af37] transition-colors px-1.5 py-1 rounded hover:bg-[#1e1e1e]"
                        title="Modifica URL"
                      >
                        ✎
                      </button>
                  ...
                  {editingSource === s.name && (
                    <div className="border-t border-[#1e1e1e] px-3 py-3 flex flex-col gap-2 bg-[#0a0a0a]">
                      <div>
                        <label className="text-xs text-[#555] mb-1 block">URL pagina casting (opzionale)</label>
                        <input type="url" value={editForm.url} onChange={e => setEditForm(f => ({ ...f, url: e.target.value }))}
                          placeholder="https://..." className={INPUT} />
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => handleSaveEdit(s.name)} disabled={editSaving}
                          className="text-xs px-3 py-1.5 rounded-lg bg-[#d4af37] text-black font-semibold hover:bg-[#b8962c] disabled:opacity-50">
                          {editSaving ? 'Salvo...' : 'Salva'}
                        </button>
                        <button onClick={() => setEditingSource(null)}
                          className="text-xs px-3 py-1.5 rounded-lg border border-[#2a2a2a] text-[#666] hover:text-[#999]">
                          Annulla
                        </button>
                      </div>
                    </div>
          ...
          <section className="bg-[#141414] border border-[#1e1e1e] rounded-xl p-5">
            ...
            <p className="text-xs text-[#444] mb-4">Aggiunti manualmente — i risultati dipendono dalla struttura del sito.</p>
              ...
              {customSources.map(s => (
                <div key={s.name} className="bg-[#0f0f0f] border border-[#1e1e1e] rounded-lg overflow-hidden">
                  ...
                  <div className="flex items-center gap-3 px-3 py-2.5">
                    <div className="flex-1 min-w-0">
                      ...
                      {s.url && (
                        <a href={s.url} target="_blank" rel="noopener noreferrer"
                          className="text-xs text-[#444] hover:text-[#d4af37] truncate block mt-0.5 max-w-[220px]">
                          {s.url}
                        </a>
                      )}
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <Toggle
                        ...
                        onChange={v => handleToggleSource(s.name, v)}
                      />
                      <button onClick={() => handleDeleteSource(s.name)} disabled={deletingSource === s.name}
                  ...
                  {editingSource === s.name && (
                    <div className="border-t border-[#1e1e1e] px-3 py-3 flex flex-col gap-2 bg-[#0a0a0a]">
                      <div>
                        <label className="text-xs text-[#555] mb-1 block">URL</label>
                        <input type="url" value={editForm.url} onChange={e => setEditForm(f => ({ ...f, url: e.target.value }))}
                          placeholder="https://..." className={INPUT} />
                      </div>
                      <div>
                        <label className="text-xs text-[#555] mb-1 block">Zona</label>
                        <select value={editForm.region} onChange={e => setEditForm(f => ({ ...f, region: e.target.value }))} className={INPUT}>
            ...
            <div className="border-t border-[#1e1e1e] pt-4 flex flex-col gap-2">
              ...
              <input type="text" placeholder="Nome (es. MyCasting)" value={newSource.name}
                onChange={e => setNewSource(s => ({ ...s, name: e.target.value }))} className={INPUT} />
              <input type="url" placeholder="URL pagina casting" value={newSource.url}
                onChange={e => setNewSource(s => ({ ...s, url: e.target.value }))} className={INPUT} />
              <select value={newSource.region} onChange={e => setNewSource(s => ({ ...s, region: e.target.value }))} className={INPUT}>
                <option value="Italy">🇮🇹 Italia</option>
                <option value="Europe">🇪🇺 Europa</option>
                <option value="International">🌍 Internazionale</option>
              </select>
              {addError && <p className="text-xs text-red-400">{addError}</p>}
              <button onClick={handleAddSource} disabled={addingSource || !newSource.name.trim() || !newSource.url.trim()}
              ...
              <p className="text-xs text-[#333]">
                ⚠ I siti aggiunti manualmente usano uno scraper generico.
              </p>
            </div>
          </section>
...
function Toggle({ enabled, loading, onChange }: { enabled: boolean; loading: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      disabled={loading}
      title={enabled ? 'Attivo — clicca per disattivare' : 'Disattivo — clicca per attivare'}
